...
# just these file are downloading from servers
file_regs = [
    r'^B.*mp4$',
    r'.mkv$',
    r'.mp4$',
    ...
    async def download_file(self, path, filename=None, token=None):
        ...
        async with aiohttp.ClientSession(trust_env=True) as session:
            ...
            if total_size_in_bytes == 0:
                return f"{filename}"
            logger.debug(total_size_in_bytes)
        ...
        for f_reg in file_regs:
            for file in fls:
                ...
                if re.search(f_reg, fname) or os.path.exists(fpath_part):
                    if os.path.exists(fpath):
                        # downloaed
                        logger.debug(f'{fpath} downloaed')
                        await self.delete_resource(path)
                        logger.debug(f'{fpath} deleted')
                        continue
                    else:
                        logger.debug(f'{fpath} downloading')
...
def scan_rar_files():
    ...
    for fname in os.listdir('tss'):
        ...
        if fname.endswith('.rar') and not os.path.exists(name):
            ...
            logger.debug(f'{src} done!')


def main():
    servers = [
            ['http://xxx', 'username', 'password'],
    ]
