...
1. 安装 Ghostty。macOS 可以直接下载官方安装包，也可以通过 Homebrew 安装：
   ...
   [Ghostty 官方安装文档](https://ghostty.org/docs/install/binary)。
...
2. 安装配置中用到的字体 FantasqueSansM Nerd Font。macOS 直接用 Homebrew：
   ...
   Linux 可从 [Nerd Fonts 下载页](https://www.nerdfonts.com/font-downloads)
   下载后放入 `~/.local/share/fonts`，再执行 `fc-cache -fv`。

3. 执行安装脚本，以下两种方式任选其一：

   **方式一：克隆仓库后执行**（推荐，便于之后用 `git pull` 同步配置）：
   ...
   **方式二：不克隆，直接执行**：
   ...
   ```bash
   curl -fsSL https://raw.githubusercontent.com/hotchilipowder/my_config/main/ghostty/install.sh | bash
   ```

   直接执行时，脚本在同目录找不到 `config.ghostty` 会自动从仓库下载。
   使用自己的 fork 时，可用环境变量 `MY_CONFIG_RAW_BASE` 指定 raw 地址前缀。
...
- 把已有的 `~/.config/ghostty/config.ghostty` 和旧的
  ...
  仓库一致时跳过备份）；
- 把 `config.ghostty` 复制到 `~/.config/ghostty/config.ghostty`，
  直接执行时会先自动下载；
- 删除 macOS 下 Application Support 目录中的空配置文件；
- 调用 `ghostty +validate-config` 校验配置语法。

不使用脚本时，也可以在克隆后的仓库根目录手动执行：
...
完整的配置路径、语法和选项说明见
[Ghostty 官方配置文档](https://ghostty.org/docs/config)。
